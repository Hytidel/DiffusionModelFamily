"""
Modified from `https://github.com/ChenyangSi/FreeU`. 
"""
...
def Fourier_filter(
    ...
    # FFT
    # x_freq.shape = (batch_size, num_channel, height, width)
    x_freq = fft.fftn(
        x, 
        dim = (-2, -1)  # (h, w)
    )
    ...
    batch_size, num_channel, height, width = x_freq.shape
    ...
    mask[
        ..., 
        half_height - threshold: half_height + threshold, 
    ...
    x_freq = fft.ifftshift(
        x_freq, 
        dim = (-2, -1)  # (h, w)
    )
    
    # IFFT
    # x_filtered.shape = (batch_size, num_channel, height, width)
    x_filtered = fft.ifftn(
        x_freq, 
        dim = (-2, -1)
    ).real
...
def register_free_crossattn_upblock2d(
    model, 
    b1 = 1.2, s1 = 0.9, 
    b2 = 1.4, s2 = 0.2
):
